                    ...
                    <tbody class="bg-white text-center" id="dosenTableBody">
                        @foreach ($kampus as $index => $item)
                            <tr class="border-b border-gray-200">
                                ...
                                <td class="p-2">
                                    <button type="button" data-modal-target="#edit-item-modal-{{ $item->id_kampus }}"
                                    ...
                                    <form id="delete-form-{{ $item->id_kampus }}"
                                        action="{{ route('adminKampus.destroy', $item->id_kampus) }}" method="POST"
                                        class="inline-block">
                                        @csrf
                                        @method('DELETE')
                                        <button type="button"
                                            ...
                                            onclick="confirmDelete('{{ $item->id_kampus }}', '{{ $item->nama_kampus }}')">
                            ...
                            <!-- Modal Edit Dosen -->
                            <div id="edit-item-modal-{{ $item->id_kampus }}" tabindex="-1" aria-hidden="true"
                                ...
                                <div class="relative w-full max-w-full md:max-w-md h-full max-h-full md:h-auto">
                                    <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                                        <button type="button"
                                            ...
                                            data-modal-hide="#edit-item-modal-{{ $item->id_kampus }}">
                                        ...
                                        <div class="p-6 text-center">
                                            <h3 class="text-lg font-semibold text-gray-900">Edit Kampus
                                                {{ $item->nama_kampus }}</h3>
                                            <form action="{{ route('adminKampus.update', $item->id_kampus) }}"
                                                ...
                                                <div class="text-left">
                                                    <label for="kd_kampus"
                                                        ...
                                                        Kampus</label>
                                                    <input type="text" name="kd_kampus" id="kd_kampus"
                                                        value="{{ $item->kd_kampus }}"
                                                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 mt-1">
                                                </div>
                                                <div class="text-left mt-4">
                                                    <label for="nama_kampus"
                                                        class="block text-sm font-medium text-gray-900">Nama Kampus</label>
            ...
            <div class="relative p-4 w-full max-w-md max-h-full">
                <div class="relative bg-white rounded-lg shadow-lg">
                    <div class="flex items-center justify-between p-4 border-b border-gray-200">
                        <h3 class="text-lg font-semibold text-black">Tambah Kampus</h3>
                        <button type="button"
                    ...
                    <form action="" method="POST" class="p-4">
                        ...
                        <div class="text-left">
                            ...
                            <input type="text" name="kd_kampus" id="kd_kampus"
                                class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 mt-1"
                                placeholder="Masukkan Kode Kampus" required>
                        </div>
                        <div class="text-left mt-4">
                            <label for="nama_kampus" class="block text-sm font-medium text-gray-900">Nama Kampus</label>
                            <input type="text" name="nama_kampus" id="nama_kampus"
                                class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 mt-1"
                                placeholder="Masukkan Nama Kampus" required>
                        </div>
                        <div class="text-left mt-4">
                            <label for="alamat" class="block text-sm font-medium text-gray-900">Alamat
                                Kampus</label>
                            <input type="text" name="alamat" id="alamat"
                                class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 mt-1"
        ...
        @if ($kampus->total() > 5)
            <div class="flex flex-col items-center my-6">
                <span class="text-sm text-gray-700 dark:text-gray-400">
                    Menampilkan <span
                        class="font-semibold text-gray-900 dark:text-white">{{ $kampus->firstItem() }}</span>
                    sampai
                    <span class="font-semibold text-gray-900 dark:text-white">{{ $kampus->lastItem() }}</span> dari <span
                        class="font-semibold text-gray-900 dark:text-white">{{ $kampus->total() }}</span> kampus
                </span>
                <div class="inline-flex mt-2 xs:mt-0">
                    <button {{ $kampus->onFirstPage() ? 'disabled' : '' }}
                        class="flex items-center justify-center px-3 h-8 text-sm font-medium text-white bg-gray-800 rounded-s hover:bg-gray-900 dark:bg-gray-800 dark:border-gray-700 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-white"
                        {{ $kampus->previousPageUrl() ? 'onclick=window.location.href=\'' . $kampus->previousPageUrl() . '\'' : '' }}>
                        Sebelumnya
                    </button>
                    <button {{ !$kampus->hasMorePages() ? 'disabled' : '' }}
                        class="flex items-center justify-center px-3 h-8 text-sm font-medium text-white bg-gray-800 border-0 border-s border-gray-700 rounded-e hover:bg-gray-900 dark:bg-gray-800 dark:border-gray-700 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-white"
                        {{ $kampus->nextPageUrl() ? 'onclick=window.location.href=\'' . $kampus->nextPageUrl() . '\'' : '' }}>
                        Selanjutnya
                    </button>
                </div>
            </div>
    ...
    <script>
        ...
        function confirmDelete(id, namaKampus) {
            Swal.fire({
                ...
                text: "Anda tidak akan dapat mengembalikan Kampus " + namaKampus,
                icon: 'warning',
